...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == []:
            if amount==0:
                for fruit_name, cnt in cart:
                    print(display(fruit_name, cnt), end='')
                print() 
            return
        fruit = fruits[0]
        price =prices[fruit]
        rest=fruits[1:]
        minimal_remaining=sum(prices[f] for f in rest)
        if rest:
            max_k=(amount-minimal_remaining)//price
        else:
            max_k=amount//price
        for k in range(1,max_k+1):
            ...
            if new_amount<0:
                continue
            add(rest,new_amount,cart+[(fruit,k)])
...
def distance(city_a, city_b):
    ...
    m_b=get_lat(city_b)
    n_b=get_lon(city_b)
    return sqrt((m_a-m_b)**2+(n_a-n_b)**2)
