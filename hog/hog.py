...
def boar_brawl(player_score, opponent_score):
    ...
    add=3*abs(tens_digit-ones_digit)
    if add:
        return add
    else:
        return 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls==0:
        win=boar_brawl(player_score,opponent_score)
    else:
        win=roll_dice(num_rolls,dice)
    return win
    # END PROBLEM 3
...
def sus_points(score):
    ...
    "*** YOUR CODE HERE ***"
    new_score=score
    while num_factors(score)==3 or num_factors(score)==4:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max1=-1.0
    for i in range(1,11):
        av=make_averaged(lambda :roll_dice(i,dice),times_called)
        aver[i-1]=av()
        if max1 < aver[i-1]:
            max1=aver[i-1]
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    add=boar_brawl(score,opponent_score)
    if add>=threshold:
        num_rolls=0
