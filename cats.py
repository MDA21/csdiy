...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    a,b=limit+0.1,typed_word
    if typed_word in word_list:
        return typed_word
    else:
        for x in word_list:
            c=diff_function(typed_word,x,limit)
            if c<a:
                a=c
                b=x
        return b
...
def furry_fixes(typed, source, limit):
    ...
    if typed[0] ==source[0]:
        return myfunc(typed[1:], source[1:], 0)
    else:
        return myfunc(typed[1:], source[1:], 1)
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    def myfunc(start,goal,ans):
        if ans>limit:# Base cases should go here, you may add more base cases as needed.
        # BEGIN
            "*** YOUR CODE HERE ***"
            return ans+1
        # END
    # Recursive cases should go below here
        if start=="":
            return len(goal)+ans
        if goal=="":
            return len(start)+ans
        if start[0]==goal[0]: # Feel free to remove or add additional cases
        # BEGIN
            "*** YOUR CODE HERE ***"
            return myfunc(start[1:],goal[1:],ans)
        # END
        else:      
        # BEGIN
            "*** YOUR CODE HERE ***"
            return min(myfunc(start[1:], goal, ans + 1), myfunc(goal[0] + start[0:], goal, ans + 1), myfunc(start[1:], goal[1:], ans + 1))
        # END
    return myfunc(typed,source,0)
